...
def ai_generate_suggestion(note):
    ...
    # 🎓 Learning Assistant
    if any(keyword in note_lower for keyword in ["learn", "how to", "study", "course", "tutorial", "want to learn"]):
        role = (
    ...
    # 😊 Mood Assistant (default)
      # 😊 Mood Assistant
        ...
        role = (
            ...
            "suggest songs that has popular and more user listen to the songs in previous two year if the user want to listen specifce song then gives it a link and a playlist otherwise gives 12 youtube song video links  and 8 best playlist song links based on user query one calming activity, a motivational book, and helpful mental health advice to feel better in 10 minutes."
        )
...
def fetch_youtube_results(query, max_results=5, sort_by_views=False):
    try:
...
def full_learning_support(note):
    ai_summary = ai_generate_suggestion(note)  # ↪️ AI generates advice based on mood note
    video_links = fetch_youtube_results(f"{note} tutorial", max_results=10, sort_by_views=True)
    playlist = fetch_playlist(f"{note} tutorial")
    doc_link = fetch_docs_link(note)
